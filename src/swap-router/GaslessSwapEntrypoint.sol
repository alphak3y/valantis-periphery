...
contract GaslessSwapEntrypoint is IGaslessSwapEntrypoint, Ownable {
    ...
    error GaslessSwapEntrypoint__swapOwnerExecute_onlySwapIntentOwner();
    ...
    /**
     * @notice Allows a user to bundle approvals before executing a gasless swap.
     * @param _gaslessSwapParams Parameters for ValantisSwapRouter::gaslessSwap.
     * @param _ownerSignature Parameters for ValantisSwapRouter::gaslessSwap.
     * @param _fee Parameters for Valantis SwapRouter::gaslessSwap.
     * @param _tokenPermitInfo Parameters for token's permit approval, if supported.
     * @param _permit2Info Parameters for Permit2's approval to ValantisSwapRouter.
     * @return amountOut Amounts of output token sent to recipient.
     */
    function swapOwnerExecute(
        ...
        if (msg.sender != _gaslessSwapParams.intent.owner) {
            revert GaslessSwapEntrypoint__swapOwnerExecute_onlySwapIntentOwner();
        }
    ...
    function _handleTokenPermit(
        ...
        if (token == DAI) {
            (uint256 nonce, uint8 v, bytes32 r, bytes32 s) =
                abi.decode(tokenPermitInfo.data, (uint256, uint8, bytes32, bytes32));
